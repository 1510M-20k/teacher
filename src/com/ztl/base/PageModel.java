...
public class PageModel {
	...
	private int txtCount; //文本中记录每页条数;
	

	
	public PageModel(){
		try {
			txtCount  = Integer.parseInt(Tools.readTxtFile(Const.PAGESIZE));
			this.showCount = txtCount;
		} catch (Exception e) {
			this.showCount = 15;
		}
	}
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			if(txtCount % 10 > 0){
				sb.append("	<option value='"+txtCount+"'>"+txtCount+"</option>\n");
			}
			for(int i=1;i<10;i++){
				int cn = 10*i;
				if(showCount == cn){
