 ...
 */	
	...
	function save(){
		...
		bootbox.confirm('确定要删除选中的数据吗?', function(result) {
			if(result){
				$("#Form").submit();
				bootbox.alert("提交成功,等待下载!",function(){
					api.close();
				});
			}
		});
	...
	function dialog_open(){
		...
		$("#form-field-radio5").prop("checked",true);
		$("#dialog-add").css("display","block");
		$("#ddefault").prop("disabled",true);
	}
	//打开编辑属性(修改)
	function editField(value,msgIndex){
		...
		if(efieldarray[1] == 'String'){
			...
			$("#form-field-radio2").prop("checked",true);
			$("#dtype").val('Integer');
		}else if(efieldarray[1] == 'Money'){
			$("#form-field-radio3").prop("checked",true);
			$("#dtype").val('Money');
		}else{
			$("#form-field-radio4").prop("checked",true);
			$("#dtype").val('Date');
		}
		if(efieldarray[3] == '是'){
			$("#form-field-radio5").prop("checked",true);
			...
			$("#form-field-radio6").prop("checked",true);
			$("#isQian").val('否');
		}
		$("#dialog-add").css("display","block");
	}
